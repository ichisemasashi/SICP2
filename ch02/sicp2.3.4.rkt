...
;手続き decode-1 は、残りのビットのリストと、木の中での現在位置という、二つの引数を取ります。
;リストの次のビットが 0 か 1 かによって、左か右の枝を “下りて” いきます (これは手続き choose-branch で行っています)。
;葉にたどり着くと、木の根から始めてメッセージの残りを復号した結果に葉の記号を cons することで、
;メッセージの次の記号として返します。choose-branch の最終節のエラーチェックに注意してください。
;ここでは、入力データの中の 0 または 1 以外のものを手続きが見つけた場合にエラーを出します。

;;;;;;;;;;
; 重みつき要素の集合

;この木の表現では、葉でないノードはそれぞれ記号の集合を持つようになっていて、
;その集合は単純なリストとして表現してきました。しかし、上で検討した木の生成アルゴリズムでは、
;葉と木の集合も扱って、小さいほうから二つの項目を順にくっつけていくという必要がありました。
;集合の最小項目を何度も求めることになるので、この種の集合は順序つきの表現を使うほうが便利です。

; 葉と木の集合は、重みの昇順に配置した要素のリストとして表現することにします。
;以下の adjoin-set 手続きは集合を構築するためのものですが、これは 練習問題 2.61で説明したものと似ています。
;しかし、項目の比較は重みによって行い、また追加する要素がすでに集合にあるということはありません。
...
; 次の手続きは、((A 4) (B 2) (C 1) (D 1)) のような記号と頻度のペアのリストを取り、
;初期状態での葉の順序つき集合を構築し、ハフマンアルゴリズムに従ってすぐ連結できるようにします。
...
;> (prn (make-leaf-set '((A 4) (B 2) (C 1) (D 1))))
